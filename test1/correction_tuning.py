...
CALIBRATION_IMAGES = [
    CalibrationImage(
        path=r"C:\fibo\3rd year_1st semester\studio\FRA362-Studio-V\test_photo\red\7M309873.jpg",
    ...
    CalibrationImage(
        path=r"C:\fibo\3rd year_1st semester\studio\FRA362-Studio-V\test_photo\red\7M309879.jpg",
    ...
    CalibrationImage(
        path=r"C:\fibo\3rd year_1st semester\studio\FRA362-Studio-V\test_photo\red\7M309883.jpg",
        actual_defect_width_mm=10.0,  # REPLACE with actual measured width
    ...
    CalibrationImage(
        path=r"C:\fibo\3rd year_1st semester\studio\FRA362-Studio-V\test_photo\red\7M309888.jpg",
...
def find_optimal_correction_factor_polynomial(calib_images, params_template,
                                              search_range=(0.1, 5.0),
                                              initial_guess_width=1.0,
                                              initial_guess_depth=1.0,
                                              sample_points=15):
    """
    Find optimal correction factors by fitting polynomial surface to error landscape.
    
    This method:
    1. Samples the error space on a grid
    2. Fits a 2D polynomial to model the error surface
    3. Finds the minimum of the polynomial analytically or numerically
    4. Refines with local optimization
    
    Better for noisy error functions or when the error landscape is complex.
    """
    from scipy.optimize import minimize
    from scipy.interpolate import Rbf
    
    print(f"\n{'='*70}")
    print("  POLYNOMIAL SURFACE FITTING CALIBRATION")
    print(f"{'='*70}")
    print(f"\nStrategy:")
    print(f"  1. Sample error at {sample_points}x{sample_points} grid points")
    print(f"  2. Fit smooth polynomial surface")
    print(f"  3. Find minimum of fitted surface")
    print(f"  4. Refine with local optimization")
    
    # Sample grid
    print(f"\nPhase 1: Sampling error landscape...")
    width_samples = np.linspace(search_range[0], search_range[1], sample_points)
    depth_samples = np.linspace(search_range[0], search_range[1], sample_points)
    
    errors = np.zeros((sample_points, sample_points))
    total_samples = sample_points * sample_points
    
    for i, w in enumerate(width_samples):
        for j, d in enumerate(depth_samples):
            sample_num = i * sample_points + j + 1
            print(f"  Sampling {sample_num}/{total_samples}: W={w:.3f}, D={d:.3f}", end='\r')
            
            error, _, _, _ = calculate_error_metrics(
                calib_images, w, d, params_template, verbose=False
            )
            errors[i, j] = error
    
    print(f"\n  Completed {total_samples} samples")
    
    # Flatten for fitting
    W_flat = np.repeat(width_samples, sample_points)
    D_flat = np.tile(depth_samples, sample_points)
    E_flat = errors.flatten()
    
    print(f"\nPhase 2: Fitting RBF surface to error landscape...")
    
    # Use Radial Basis Function interpolation (smooth surface)
    rbf = Rbf(W_flat, D_flat, E_flat, function='multiquadric', smooth=0.1)
    
    # Find minimum on the fitted surface
    def surface_error(factors):
        w, d = factors
        return rbf(w, d)
    
    # Find approximate minimum from samples
    min_idx = np.unravel_index(np.argmin(errors), errors.shape)
    approx_min_w = width_samples[min_idx[0]]
    approx_min_d = depth_samples[min_idx[1]]
    
    print(f"  Approximate minimum from samples: W={approx_min_w:.3f}, D={approx_min_d:.3f}")
    
    print(f"\nPhase 3: Refining with local optimization...")
    
    # Refine using the smooth surface
    from scipy.optimize import Bounds
    bounds = Bounds([search_range[0], search_range[0]], 
                   [search_range[1], search_range[1]])
    
    result_surface = minimize(
        surface_error,
        x0=[approx_min_w, approx_min_d],
        method='L-BFGS-B',
        bounds=bounds,
        options={'ftol': 1e-5, 'maxiter': 30}
    )
    
    optimal_w_surface = result_surface.x[0]
    optimal_d_surface = result_surface.x[1]
    
    print(f"  Surface minimum: W={optimal_w_surface:.3f}, D={optimal_d_surface:.3f}")
    
    print(f"\nPhase 4: Final verification with actual error function...")
    
    # Verify with actual error (not RBF approximation)
    final_error, mean_w_err, mean_d_err, _ = calculate_error_metrics(
        calib_images, optimal_w_surface, optimal_d_surface, params_template, verbose=False
    )
    
    print(f"\n{'='*70}")
    print("  OPTIMIZATION COMPLETE")
    print(f"{'='*70}")
    print(f"\nOptimal Width Factor:  {optimal_w_surface:.3f}")
    print(f"Optimal Depth Factor:  {optimal_d_surface:.3f}")
    print(f"Width error: {mean_w_err:.2f} mm")
    print(f"Depth error: {mean_d_err:.2f} mm")
    print(f"Total error: {final_error:.2f} mm")
    
    return optimal_w_surface, optimal_d_surface, final_error


def find_optimal_correction_factor_grid_search(calib_images, params_template,
                                               search_range=(0.1, 5.0),
                                               initial_guess_width=1.0,
                                               initial_guess_depth=1.0,
                                               coarse_samples=10,
                                               fine_samples=5):
    """
    Find optimal correction factors using hierarchical grid search.
    
    Strategy:
    1. Coarse grid search over full range
    2. Fine grid search around best coarse result
    3. Very fine local optimization
    
    Most robust but slower - guaranteed to explore the space thoroughly.
    """
    print(f"\n{'='*70}")
    print("  HIERARCHICAL GRID SEARCH CALIBRATION")
    print(f"{'='*70}")
    print(f"\nStrategy: Coarse → Fine → Local optimization")
    
    # Phase 1: Coarse search
    print(f"\nPhase 1: Coarse grid ({coarse_samples}x{coarse_samples} = {coarse_samples**2} points)")
    width_coarse = np.linspace(search_range[0], search_range[1], coarse_samples)
    depth_coarse = np.linspace(search_range[0], search_range[1], coarse_samples)
    
    best_error = float('inf')
    best_width = initial_guess_width
    best_depth = initial_guess_depth
    
    for i, w in enumerate(width_coarse):
        for j, d in enumerate(depth_coarse):
            progress = (i * coarse_samples + j + 1)
            print(f"  Testing {progress}/{coarse_samples**2}: W={w:.3f}, D={d:.3f}", end='\r')
    ...
    print(f"\n  Coarse best: W={best_width:.3f}, D={best_depth:.3f}, Error={best_error:.2f}mm")
    
    # Phase 2: Fine search around best point
    print(f"\nPhase 2: Fine grid around best point ({fine_samples}x{fine_samples} points)")
    
    # Define fine search range (20% around best point)
    width_range = (search_range[1] - search_range[0]) * 0.2
    depth_range = (search_range[1] - search_range[0]) * 0.2
    
    width_fine = np.linspace(max(search_range[0], best_width - width_range/2),
                             min(search_range[1], best_width + width_range/2),
                             fine_samples)
    depth_fine = np.linspace(max(search_range[0], best_depth - depth_range/2),
                             min(search_range[1], best_depth + depth_range/2),
                             fine_samples)
    
    for i, w in enumerate(width_fine):
        for j, d in enumerate(depth_fine):
            progress = (i * fine_samples + j + 1)
            print(f"  Testing {progress}/{fine_samples**2}: W={w:.3f}, D={d:.3f}", end='\r')
    ...
    print(f"\n  Fine best: W={best_width:.3f}, D={best_depth:.3f}, Error={best_error:.2f}mm")
    
    # Phase 3: Local optimization
    print(f"\nPhase 3: Local optimization polish...")
    ...
    result = minimize(
        objective,
        x0=[best_width, best_depth],
        method='L-BFGS-B',
        bounds=bounds,
        options={'ftol': 1e-4, 'maxiter': 20}
    )
    
    optimal_width = result.x[0]
    optimal_depth = result.x[1]
    final_error = result.fun
    
    print(f"\n{'='*70}")
    print("  OPTIMIZATION COMPLETE")
    print(f"{'='*70}")
    print(f"\nOptimal Width Factor:  {optimal_width:.3f}")
    print(f"Optimal Depth Factor:  {optimal_depth:.3f}")
    print(f"Final error: {final_error:.2f} mm")
    
    return optimal_width, optimal_depth, final_error
...
def main():
    ...
    print(f"\n3. Polynomial Surface - Fits smooth surface to error landscape")
    print(f"   Best for: Noisy data, complex error surfaces")
    print(f"\n4. Grid Search - Most thorough but slowest")
    print(f"   Best for: When other methods fail, guaranteed exploration")
    
    method_choice = input(f"\nSelect method (1-4) or press Enter for default [1]: ").strip()
    ...
    if method_choice == '2':
        ...
        print(f"\n→ Using Polynomial Surface Fitting")
        optimal_factor_width, optimal_factor_depth, final_error = find_optimal_correction_factor_polynomial(
            valid_images, 
            params_template,
            search_range=search_range,
            initial_guess_width=params_template.correction_factor_width,
            initial_guess_depth=params_template.correction_factor_depth,
            sample_points=12  # Adjust for speed vs accuracy
        )
    elif method_choice == '4':
        print(f"\n→ Using Hierarchical Grid Search")
        optimal_factor_width, optimal_factor_depth, final_error = find_optimal_correction_factor_grid_search(
            valid_images, 
            params_template,
            search_range=search_range,
            initial_guess_width=params_template.correction_factor_width,
    ...
    print(f"\n✓ Optimal Width Factor: {optimal_factor_width:.3f}")
    print(f"✓ Optimal Depth Factor: {optimal_factor_depth:.3f}")
    print(f"\nTo use this calibration, update the Params class with:")
